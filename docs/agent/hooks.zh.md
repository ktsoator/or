...
零值都是“无钩子”，所以裸 agent 就是一个朴素的工具循环。这些钩子让你拦截工具调用、在步骤
...
PrepareNextStep          （该步骤及其工具结果追加之后）
ShouldStopAfterStep      （下一次请求开始之前）
```
...
`PrepareNextStep` 在每个步骤后运行，可为下一步骤替换模型、思考强度或上下文。由于历史按
...
`StepUpdate` 携带可选的 `Context`、`Model` 和 `ThinkingLevel`；nil 字段保留当前值。
`StepCtx` 把该步骤的 assistant 消息、它的工具结果、当前上下文，以及 `NewMessages`——
即运行此刻停止会返回的内容——交给钩子。

## 提前停止

`ShouldStopAfterStep` 在下一次请求开始之前请求一次优雅停止。agent 没有内置的步骤上限，
所以这里就是你防止失控循环的地方。
