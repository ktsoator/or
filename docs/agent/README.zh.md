...
——同时把历史存储和上下文压缩留给调用方处理。

它是一个与厂商无关的编排层：一个无状态引擎（`RunLoop`），外加一个可选的有状态封装
（`Agent`），扩展点以函数字段的形式提供。它不捆绑具体的工具、持久化方案或 system 提示。
...
一个工具由 `llm.ToolDefinition`（从 Go 结构体派生的 JSON schema）和一个 `Execute`
函数组成。`Execute` 通过返回 error 报告失败，循环会把它转成一个错误结果，而不会中止
整个运行。可选的 `onUpdate` 回调用于流式输出部分进度。可选的 `PrepareArguments` 会在
schema 校验前重写原始参数——用它来容忍提供方的怪癖或填充默认值。

发生 panic 的工具不会让进程崩溃：循环会将其恢复成一个错误结果并继续。来自调用方钩子
（`ConvertToLLM`、`TransformContext`、`PrepareNextTurn`、`ShouldStopAfterTurn`
或工具调用钩子）的 panic 同样会被恢复，以一个终止错误事件干净地结束本次运行。
...
`New` 构建一个有状态的 agent；`Prompt` 将一个任务运行到完成，并把结果追加到它的
对话记录中。`Prompt` 接受字符串、单个 `AgentMessage` 或一个切片。对于多模态提示，
`agent.UserMessage` 可由文本和图像构建一条用户消息：
...
`Subscribe` 注册一个监听器，按顺序接收每一个事件，并返回一个用于移除该监听器的函数。
`Prompt` 会阻塞直到运行结束，因此监听器会在运行期间持续触发。
...
| 事件 | 含义 | 值得注意的字段 |
|---|---|---|
| `AgentStart` / `AgentEnd` | 运行的起止边界 | `AgentEnd.Messages` —— 本次运行追加的所有内容 |
...
`RunLoop` 是 `Agent` 底层的引擎。它接收新的提示和一个基础 context，返回一个事件
通道，并把对话记录交给你管理。最终的 `AgentEnd` 事件携带本次运行追加的消息。
...
`BeforeToolCall` 在参数校验之后、执行之前运行；返回 `block` 会跳过该工具，并将
`reason` 用作错误结果。
...
`AfterToolCall` 在执行之后运行；非 nil 的返回值会逐字段覆盖结果。若在一个批次中的每个
结果上都设置 `Terminate`，则会在该批次之后停止运行。
...
`Continue` 在没有新提示的情况下从当前对话记录恢复运行——用于重试，或在带外追加消息
之后继续。提供方要求最新一轮是用户消息或工具结果，因此当对话记录以一条 assistant 消息
结尾时，`Continue` 会回退到排队的消息：它先取出干预（steering）队列，再取出追加
（follow-up）队列，并运行其中找到的内容。仅当最后一条消息是 assistant 且两个队列都为空
时，它才会报错。
...
`Steer` 在下一轮之前注入一条消息；`FollowUp` 在 agent 本将停止之后注入一条。可在
...
`SteeringMode` 和 `FollowUpMode` 控制每次取出多少条排队消息：`QueueOneAtATime`
（默认）只注入最早的一条，其余留给后续轮次；`QueueAll` 一次性注入全部。
...
GetAPIKey: func(provider string) string {
	return currentOAuthToken(provider) // 在别处刷新
},
```
...
一次运行操作的对象是 `AgentMessage`：用 `FromLLM` 适配的标准 `llm` 消息，以及嵌入了
`Custom` 的应用类型。自定义消息会留在对话记录和事件流中，但会被默认的 `ConvertToLLM`
丢弃，因此它们永远不会到达模型。提供你自己的 `ConvertToLLM` 来转换它们。
...
assistant.Reset()              // 清空对话记录、错误和队列；保留配置
```

要在两次运行之间重新配置 agent，使用 setter。每个 setter 在下一次运行时生效，且不会
干扰已在进行中的运行：
...
可运行的程序见 [`example/agent`](https://github.com/ktsoator/or/tree/main/example/agent)：
...
环境都被有意排除在这一层之外；`TransformContext` 钩子正是日后接入压缩能力的地方。
