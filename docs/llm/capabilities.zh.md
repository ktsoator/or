...
| 使用兼容模型服务 | 手动构造 `Model` | 接入兼容 OpenAI Chat Completions 或 Anthropic Messages 接口的服务地址 | [自定义网关](recipes/custom-gateway.md) |
...
| 新增请求与响应协议 | `ProtocolAdapter`、`StreamWriter` | 实现请求转换和统一流式事件 | [自定义协议](extending.md) |
| 无真实 API 测试 | 手工 `AssistantMessage`、`httptest.Server` | 测试结果处理或完整协议路径 | [Mock Provider 测试](recipes/mock-testing.md) |

第一个完整可运行程序只在[快速开始](getting-started.md)维护。上表中的场景链接提供更完整的请求流程、结果处理和生产约束。
...
- 支持矩阵中标记为“仅目录”的协议 adapter。

这些能力可以在 `llm` 之上实现。当前材料没有说明内置实现。具体协议状态见[支持矩阵](support-matrix.md)，不要在业务代码中硬编码这里的限制列表。
...
- 模型服务的接口格式兼容现有协议：构造 `Model` 或注册 provider，无需实现新 adapter。
- 模型服务采用框架尚未支持的请求与响应格式：实现 `ProtocolAdapter`。
