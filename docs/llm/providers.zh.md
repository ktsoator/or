...
目录中还包含其他兼容提供方和模型的元数据。这些条目可被查询，并且可能可以通过两种
协议适配器之一工作，但它们并未全部针对在线提供方 API 验证过，不构成支持保证。
自动化测试使用本地 mock 服务器来覆盖两种适配器，而非针对每个提供方进行在线集成测试。

只有 `llm.GetModel` 所选提供方的 key 会被读取。也可以通过 `StreamOptions.APIKey`
或 `StreamOptions.Env` 提供请求级别的凭证。

## 发现模型

查询目录，而不是把动态提供的模型 ID 硬编码：
...
当提供方或模型 ID 不存在时会 panic。模型元数据包含推理与图像支持、上下文窗口、
输出上限和定价信息。

## 自定义与兼容端点

任何实现了内置协议之一的端点，都可以通过直接构造一个 `Model` 并设置 `BaseURL`
来使用。这涵盖了 Ollama、vLLM、LM Studio 等本地服务器，以及私有模型网关：
...
端点特定的行为——推理字段名、cache-control 支持以及类似差异——通过
`Model.Compatibility`，配合 `OpenAICompletionsCompatibility` 或
`AnthropicMessagesCompatibility` 来配置。

对于既非 OpenAI 兼容也非 Anthropic 兼容的线缆协议，请实现一个
