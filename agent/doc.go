...
// package.
//
// It turns a single model request into a complete tool-call loop: stream an
// assistant response, execute the tool calls the model requests, append the
// results, and continue until the model stops. RunLoop is the stateless engine;
// Agent is a thin stateful wrapper that adds a retained transcript, event
// subscription, and steering and follow-up queues.
//
