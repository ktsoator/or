...
export function Thinking({ item }: { item: ThinkingItem }) {
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="my-2 animate-[fade-in_160ms_ease-out] text-stone-500">
      <CollapsibleTrigger className="group flex cursor-pointer items-center gap-2 border-0 bg-transparent py-0.5 text-[13px] font-medium text-inherit hover:text-stone-900 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-slate-400">
      ...
      <CollapsibleContent>
        <div className="mt-2 max-h-56 overflow-auto border-l border-stone-200 pl-3.5 text-[13.5px] leading-[1.62] whitespace-pre-wrap text-stone-500">
          {item.text}
        </div>
      </CollapsibleContent>
    </Collapsible>
